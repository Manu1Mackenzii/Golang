...
type AuthController struct {
	...
	EventService *services.EventService // service lié aux concerts
}

// NewAuthController crée une nouvelle instance de AuthController
func NewAuthController(authService *services.AuthService, imageService *services.ImageService, eventService *services.EventService) *AuthController {
...
func (ctrl *AuthController) RegisterHandler(c *fiber.Ctx) error {
	...
	// Définir le rôle de l'utilisateur si non fourni
	if req.Role == "" {
		req.Role = "Fan" // rôle par défaut dans un contexte musical
	}
	...
	err = ctrl.AuthService.SendConfirmationEmail(user.Email, user.ConfirmationToken)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Erreur lors de l'envoi de l'email de confirmation"})
	}
...
func (ctrl *AuthController) GoogleCallback(c *fiber.Ctx) error {
	...
	// Vérifier et récupérer les informations de l'utilisateur
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Erreur lors de la récupération des informations utilisateur"})
	}
	...
	if err := json.NewDecoder(resp.Body).Decode(&userInfo); err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Erreur de décodage des informations utilisateur"})
	}

	// Vérifier si l'utilisateur existe déjà ou le créer
	user, err := ctrl.AuthService.GetUserByEmail(userInfo.Email)
	if err != nil {
		user, err = ctrl.AuthService.RegisterUser(models.Users{Email: userInfo.Email})
		if err != nil {
			return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Erreur lors de l'inscription de l'utilisateur"})
		}
	}
	...
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Erreur lors de la conversion de l'ID utilisateur"})
	}
	accessToken, err := middlewares.GenerateToken(userID)
	if err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{"error": "Erreur lors de la génération du token d'accès"})
	}

	return c.Status(fiber.StatusOK).JSON(fiber.Map{"accessToken": accessToken})
}

// GetUserInfoHandler récupère les informations de l'utilisateur
func (ctrl *AuthController) GetUserInfoHandler(c *fiber.Ctx) error {
